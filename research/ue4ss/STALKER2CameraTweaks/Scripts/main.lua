...
local lastWideAspect = nil
local sequence = 0
local gameplayStage = "idle"
local gameplayCameraIdentity = nil
local gameplayAttempts = 0
local MAX_GAMEPLAY_ATTEMPTS = 4
local cinematicLifecycleInstalled = false
local cinematicSignalSource = "none"
local cinematicSignalSeen = false
local dialogueWriteDone = false
local cinematicApplied = false
local cinematicLastAppliedFov = nil
...
local function processDialogue(camera, state)
    ...
    -- A constrained gameplay aspect transition can animate FOV. It is not
    -- dialogue zoom and must not seed the dialogue detector.
    if state.constrain == true then
        dialogueActive = false
        dialogueBaselineFov = nil
        dialogueWriteDone = false
        lastGameplayFov = state.fov
        return
    end
    if string.lower(config.dialogueZoom) == "native" then
        dialogueActive = false
        dialogueBaselineFov = nil
        lastGameplayFov = state.fov
        return
    ...
    if dialogueActive and dialogueBaselineFov then
        if state.fov > dialogueBaselineFov - 1.0 then
            ...
            -- Rewriting every poll fights the game's interpolation and can
            -- create a visible zoom-in/zoom-out loop.
            if not dialogueWriteDone then
                apply(camera, "FieldOfView", dialogueTarget(dialogueBaselineFov), nil, "dialogue-zoom")
                dialogueWriteDone = true
            end
        end
    end
    lastGameplayFov = state.fov
...
local function targetCinematicAspect()
    local policy = string.lower(config.cinematicAspect:gsub("%s+", ""))
    if policy == "native" or policy == "16:9" or policy == "16x9" then
        return NATIVE_ASPECT
    end
    if policy == "21:9" or policy == "21x9" then
        return 21.0 / 9.0
    end
    if policy == "32:9" or policy == "32x9" then
        return 32.0 / 9.0
    end
    if policy == "auto" then
        if lastWideAspect and lastWideAspect > NATIVE_ASPECT + 0.05 then
            return lastWideAspect
        end
        emit("AUTO_ASPECT_FALLBACK aspect=" .. fmt(NATIVE_ASPECT))
        return NATIVE_ASPECT
    end
    emit("CINEMATIC_ASPECT_INVALID value=" .. config.cinematicAspect .. " fallback=Native")
    return NATIVE_ASPECT
end

local function horPlus(sourceFov, aspect)
    if sourceFov == nil or aspect == nil or aspect <= 0 then return sourceFov end
    return 2.0 * math.atan(math.tan(sourceFov * math.pi / 360.0) * aspect / NATIVE_ASPECT) * 180.0 / math.pi
end

local function applyCinematic(camera, authoredFov)
    local policy = string.lower(config.cinematicAspect:gsub("%s+", ""))
    if policy == "native" then return true end
    local aspect = targetCinematicAspect()
    if authoredFov == nil then return end
    local currentAspect = number(read(camera, "AspectRatio", nil))
    if currentAspect == nil or math.abs(currentAspect - aspect) > 0.05 then
        apply(camera, "AspectRatio", aspect, "SetAspectRatio", "cinematic-aspect")
        return false
    end
    apply(camera, "bConstrainAspectRatio", true, "SetConstraintAspectRatio", "cinematic-constrain")
    apply(camera, "FieldOfView", horPlus(authoredFov, aspect), nil, "cinematic-fov")
    return true
end

local function cinematicPolicyManaged()
    return string.lower(config.cinematicAspect:gsub("%s+", "")) ~= "native"
end

local function setCinematicState(active, source)
    active = active == true
    if cinematicActive == active then return end
    cinematicActive = active
    cinematicSignalSource = source or cinematicSignalSource
    if source == "HooksModule" or source == "RegisterHook" then
        cinematicSignalSeen = true
    end
    cinematicBaselineFov = nil
    cinematicApplied = false
    cinematicLastAppliedFov = nil
    dialogueActive = false
    dialogueBaselineFov = nil
    dialogueWriteDone = false
    lastGameplayFov = nil
    emit("CINEMATIC_" .. (active and "ENTER" or "EXIT") .. " source=" .. cinematicSignalSource)
end

local function installCinematicLifecycle()
    local ok, hooks = pcall(require, "hooks")
    if ok and hooks and hooks.setCinematicCallback then
        local previous = nil
        if hooks.getCinematicCallback then
            previous = hooks.getCinematicCallback()
        end
        hooks.setCinematicCallback(function(active)
            setCinematicState(active == true, "HooksModule")
            if previous then pcall(previous, active) end
        end)
        cinematicLifecycleInstalled = true
        cinematicSignalSource = "HooksModule"
        emit("CINEMATIC_LIFECYCLE source=HooksModule")
        return true
    end

    local fallbackOk = pcall(function()
        RegisterHook("/Script/Engine.PlayerController:SetCinematicMode", function(_, _, inCinematicMode)
            local active = inCinematicMode
            if type(active) == "table" and active.get then
                active = active:get()
            end
            setCinematicState(active == true, "RegisterHook")
        end)
    end)
    cinematicLifecycleInstalled = fallbackOk
    cinematicSignalSource = fallbackOk and "RegisterHook" or "none"
    emit("CINEMATIC_LIFECYCLE source=" .. cinematicSignalSource ..
        " result=" .. (fallbackOk and "installed" or "blocked"))
    return fallbackOk
end
...
local function tick(reason)
    ...
    if wideUnconstrained then
        lastWideAspect = camera.aspect
    end

    -- Some cinematic paths do not emit the reflected SetCinematicMode hook.
    -- Only use the constrained 16:9 state as a fallback after gameplay has
    -- completed, so the gameplay two-pass is not misclassified as cinematic.
    if not cinematicSignalSeen and gameplayStage == "complete" and camera.aspect ~= nil then
        local heuristic = camera.constrain == true and
            (cinematicActive or (camera.aspect >= 1.60 and camera.aspect <= 1.95))
        if heuristic ~= cinematicActive then
            setCinematicState(heuristic, "state-fallback")
        end
    end

    if cinematicActive then
        if cinematicBaselineFov == nil then
            -- During cinematic entry FieldOfView can still contain the
            -- gameplay FOV. FirstPersonFieldOfView is the authored 90-degree
            -- source used by the proven cinematic calculation.
            cinematicBaselineFov = camera.first or camera.fov
            emit("CINEMATIC_BASELINE_FOV=" .. fmt(cinematicBaselineFov))
        end
        if not cinematicApplied then
            if applyCinematic(cache.camera, cinematicBaselineFov) then
                cinematicApplied = true
                if cinematicPolicyManaged() then
                    cinematicLastAppliedFov = horPlus(cinematicBaselineFov, targetCinematicAspect())
                end
                emit("CINEMATIC_APPLIED once=true")
            end
        elseif cinematicPolicyManaged() and cinematicLastAppliedFov ~= nil and camera.fov ~= nil and
            math.abs(camera.fov - cinematicLastAppliedFov) > EPSILON then
            -- A later native shot update is new authored input. Transform it
            -- once and remember the result so our own write is ignored.
            local authoredFov = camera.fov
            local transformedFov = horPlus(authoredFov, targetCinematicAspect())
            apply(cache.camera, "FieldOfView", transformedFov, nil, "cinematic-authored-fov")
            cinematicLastAppliedFov = transformedFov
            emit("CINEMATIC_AUTHORED_FOV input=" .. fmt(authoredFov) ..
                " output=" .. fmt(transformedFov))
        end
    else
        applyGameplay(cache.camera, camera)
        processDialogue(cache.camera, camera)
    end

    local phase = cinematicActive and "CINEMATIC" or (wideUnconstrained and "GAMEPLAY_WIDE" or "GAMEPLAY")
...
emit("loaded; gameplay/cinematic/dialogue policy branches active; runtime validation pending; End logs a marker")
